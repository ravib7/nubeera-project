...
<body>
    ...
    <div class="content-box" id="content-box">
        <div class="body-content">
            <h1 class="py-2 px-5 mb-0 heading" style="background: #33A4FF; color: white;">PySpark map() Transformation</h2>
                ...
                <div class="px-5 pb-5 pt-0 body-text">
                    ...
                    <div class="entry-content clr" itemprop="text">
                        <p>PySpark map (<code>map()</code>) is an RDD transformation that is used to apply the transformation function (lambda) on every element of RDD/DataFrame and returns a new RDD. In this article, you will learn the syntax and usage of the RDD map() transformation with an example and how to use it with DataFrame. </p>
                        <p>RDD <code>map()</code> transformation is used to apply any complex operations like adding a column, updating a column, transforming the data e.t.c, the output of map transformations would always have the same number of records as input.</p>
                        <ul>
                            <li><strong>Note1:</strong> DataFrame doesn&#8217;t have map() transformation to use with DataFrame hence you need to DataFrame to RDD first. </li>
                            <li><strong>Note2:</strong> If you have a heavy initialization use PySpark mapPartitions() transformation instead of map(), as with mapPartitions() heavy initialization executes only once for each partition instead of every record.</li>
                        </ul>
...
map(f, preservesPartitioning=False)
                        ...
                        <h2>PySpark map() Example with RDD</h2>
                        <p>In this PySpark <code>map()</code> example, we are adding a new element with value 1 for each element, the result of the RDD is <code>PairRDDFunctions</code> which contains key-value pairs, word of type String as Key and 1 of type Int as value.</p>
                        <pre><blockquote class="language-python">
...
for element in rdd2.collect():
    print(element)
                        ...
                        <img src="https://i2.wp.com/sparkbyexamples.com/wp-content/uploads/2020/08/pyspark-rdd-map.png?w=267&ssl=1" alt="pyspark rdd map transformation" style="width:50%">
                        <h2>PySpark map() Example with DataFrame</h2>
                        <p>PySpark DataFrame doesn&#8217;t have <code>map()</code> transformation to apply the lambda function, when you wanted to apply the custom transformation, you need to convert the DataFrame to RDD and apply the map() transformation. Let&#8217;s use another dataset to explain this.</p>
                        <pre><blockquote class="language-python">
...
+---------------+------+----------+
                        ...
                        <p>Note that aboveI have used index to get the column values, alternatively, you can also refer to the DataFrame column names while iterating.</p>
                        <pre><blockquote class="language-python">
...
rdd2=df.rdd.map(lambda x: func1(x))
                        ...
                        <h2>Complete PySpark map() example</h2>
                        <p>Below is complete example of PySpark map() transformation.</p>
                        <pre><blockquote class="language-python">
from pyspark.sql import SparkSession
...
rdd2=df.rdd.map(lambda x: func1(x))
                        ...
                        <p>In conclusion, you have learned how to apply a <code>map()</code> transformation on every element of PySpark RDD and learned it returns the same number of elements as input RDD. This is one of the differences between <code>map()</code> and <code>&lt;a href=&quot;https://sparkbyexamples.com/pyspark/pyspark-rdd-flatmap-transformation/&quot;&gt;flatMap()&lt;/a&gt;</code> transformations. And you have also learned how to use map() on DataFrame by converting DataFrame to RDD.</p>
                        <p>Happy Learning !!</p>
